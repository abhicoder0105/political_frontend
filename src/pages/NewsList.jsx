...
export default function NewsList() {
  const { data, loading, error } = useApi('/api/public/news')
  ...
  if (news.length === 0) return <div className="mx-auto max-w-4xl px-4 py-16"><EmptyState message="कोई समाचार नहीं" /></div>
    ...
    <div className="mx-auto max-w-4xl px-4 py-16">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        {news.map((item) => (
          ...
          >
            {item.image_url && (
              <LazyImage src={item.image_url} alt={item.title} className="h-48 w-full object-cover" />
            )}
            <div className="p-4">
              <h3 className="font-bold text-slate-900 group-hover:text-orange-600">{item.title}</h3>
              <p className="mt-2 text-sm text-slate-500 line-clamp-2">{item.content}</p>
              <p className="mt-2 text-xs text-slate-400">{new Date(item.published_at || item.created_at).toLocaleDateString()}</p>
            </div>
          </Link>
        ))}
      </div>
