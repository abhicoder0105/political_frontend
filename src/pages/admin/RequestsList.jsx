...
export default function AdminRequestsList() {
  ...
  async function handleDelete(id) {
    if (!confirm('क्या आप इस शिकायत को हटाना चाहते हैं?')) return
    await apiRequest(`/api/admin/requests/${id}`, { method: 'DELETE' })
    toast.success('शिकायत हटा दी गई')
    reload()
  }
  ...
  return (
    <div className="p-5">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-xl font-black text-slate-900">शिकायतें</h1>
      </div>
      ...
      <div className="mb-4 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-4">
        ...
        <input className="input" placeholder="गाँव / वार्ड" value={query.village_id || ''} onChange={(e) => handleFilter('village_id', e.target.value)} />
      ...
      {!loading && !error && (!data?.data || data.data.length === 0) && <EmptyState message="कोई शिकायत नहीं मिली" />}
        ...
        <div className="overflow-auto rounded-lg border border-slate-200 bg-white">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-left">
              <tr>
                {['आईडी', 'नाम', 'फोन', 'क्षेत्र', 'गाँव/वार्ड', 'श्रेणी', 'गंभीरता', 'स्थिति', 'कार्यकर्ता', 'दिनांक', 'क्रियाएँ'].map((h) => (
                  <th key={h} className="px-3 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.data.map((req) => (
                <tr key={req.id} className="border-t">
                  ...
                  <td className="px-3 py-3">{new Date(req.created_at).toLocaleDateString()}</td>
                  <td className="px-3 py-3 space-x-2">
                    <a href={`/admin/requests/${req.id}`} className="font-bold text-emerald-700 hover:underline">View</a>
                    <button className="font-bold text-orange-600 hover:underline" onClick={() => setEditing(req)}>Edit</button>
                    <button className="font-bold text-red-600 hover:underline" onClick={() => handleDelete(req.id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {editing && (
        <EditRequestModal
          ...
          onSave={async (formData) => {
            await handleQuickUpdate(`/api/admin/requests/${editing.id}`, { public_request: formData }, 'शिकायत अपडेट की गई')
            setEditing(null)
          }}
        />
      )}
...
function EditRequestModal({ request, onSave, onClose, submitting }) {
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/40 p-4">
      ...
      >
        <h2 className="text-xl font-black">शिकायत संपादित करें</h2>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <label>
            ...
            <select className="input" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value, escalated: e.target.value === 'escalated' })}>
              {REQUEST_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </label>
          ...
          <label className="md:col-span-2">
            ...
            <textarea className="input min-h-20" value={form.internal_notes || ''} onChange={(e) => setForm({ ...form, internal_notes: e.target.value })} />
          </label>
          <label className="md:col-span-2">
            <span className="label">सार्वजनिक प्रतिक्रिया</span>
            <textarea className="input min-h-20" value={form.public_response || ''} onChange={(e) => setForm({ ...form, public_response: e.target.value })} />
          </label>
        </div>
